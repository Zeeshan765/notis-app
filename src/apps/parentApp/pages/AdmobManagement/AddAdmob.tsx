...
const AddAdmob = () => {
  ...
  const onSubmit = async (values: any) => {
    try {
      setLoading(true);
      const response = await apiService.post('api/v1/admin/adMob', values);
  ...
  return (
    <>
      <Breadcrumb pageName={t('AddAdMob')} parentName={t('AdMobManagement')} />
            ...
            <form onSubmit={formik.handleSubmit}>
              <div className="p-6.5">
                ...
                <div className="flex flex-col md:flex-row">
                  ...
                  >
                    {loading ? <Spinner className="h-6 w-6" /> : `${t('CreateAdMob')}`}
                  </button>
                  <button
