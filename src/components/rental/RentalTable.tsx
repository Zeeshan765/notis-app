...
const RentalTab: React.FC<Props> = ({
  userData,
  handleChange,
  handleSort,
  handlePopup,
  ...
  const handleDelete = async (id: any) => {
    try {
      setIsDelete(true);
      const response = await apiService.delete(`api/v1/admin/rentals/${id}`);
  ...
  const handleEdit = (id: string) => {
    navigate(`/panel/rentals/edit-rental/${id}`);
  };
