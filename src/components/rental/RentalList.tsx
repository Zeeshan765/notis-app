...
import DataTable from './RentalTable';
...
const RentalList: React.FC = () => {
  ...
  const getApiData = async (params: any) => {
    try {
      setLoading(true);
      const response = await apiService.get(`api/v1/admin/rentals`, params);
      const { records } = response.data.data.rentals;
      setUserData(records);
      setLoading(false);
    } catch (error) {
      setLoading(false);
  ...
  const getPaginatedData = async (params: any) => {
    try {
      setLoading(true);
      const response = await apiService.get(`api/v1/admin/rentals`, params);
      const { records, metadata } = response.data.data.rentals;
