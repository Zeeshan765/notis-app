...
const EditImageSelect: React.FC<Props> = ({
  onChange,
  fileData,
  setFileData,
  parentId,
  ...
  // const [setFileData, setSelectedImages] = useState<string[]>([]);
  ...
  const handleFileChange = (event: ChangeEvent<HTMLInputElement>) => {
    if (event.target.files && event.target.files.length > 0) {
      ...
      const fileUrls = filesArray.map((file) => file);
      console.log('fileUrls', fileUrls);

      const newFilesData = filesArray.map((file, index) => ({
        _id: '', 
        source: 'local',
        path: fileUrls[index],
        action:'add'
      ...
      setFileData((prevData: any) => {
        const updatedData = [...prevData, ...newFilesData];
  ...
  const handleRemoveImage = async (index: number) => {
    const urlToRemove = fileData[index];
    if (urlToRemove._id) {
      const formData = new FormData();
      fileData.forEach((url: any, index: number) => {
      ...
      const updatedImages = fileData.filter((_: any, i: any) => i !== index);
      const updatedFiles = fileData.filter((_: any, i: any) => i !== index);
      setFileData(updatedImages);
  ...
  return (
    <div className="flex items-center mt-1">
      ...
      <label htmlFor="imageInput" className="cursor-pointer">
        ...
        >
          {fileData.length > 0 ? `${t('AddMore')}` : `${t('UploadImage')}`}
        </button>
      </label>
      {fileData.length > 0 ? (
        fileData.map((image: any, index: any) => (
...
export default EditImageSelect;
